...
class Main6 {
    ...
    static void printInorder(Node12 root) {
        if (root == null) return;
...
class Tree4 {
    ...
    public static Node12 deleteNode(Node12 root, int X) {
        if(root == null){
            return null;
        }
        //traverse thru the tree to find the node to be deleted
        if(X < root.data){
    ...
    //basically taking the left-most (aka smallest) element from tree with node given as the root
    static int minValue (Node12 root){
        int minv = root.data;
